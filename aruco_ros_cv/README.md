...
same subscription instead of creating a new one; only brand-new topics count
against `max_processing_threads`. The shared subscription callback only
decodes each incoming frame; every detection then runs on its **own dedicated
worker thread**, so multiple detections on the same topic process each frame
in parallel instead of queuing behind one another.

- **Action** `~/start_detection` (`aruco_ros_cv_interfaces/action/ArucoRtStart`)
  Starts a detection on a camera topic.
  - Goal: `string image_topic`, `string[] dictionaries`, `float64[] marker_sizes`, `sensor_msgs/CameraInfo camera_info`, `string detection_id`, `int32[] marker_ids`
  - Result: `bool success`, `string message`, `string detection_id`
  - Feedback: `string status`, `uint32 active_threads`
  - `detection_id` is optional — if empty, it's auto-generated from `dictionaries` (joined with `_`) plus a suffix describing `marker_ids` (see below), and the resolved id is always echoed back in `Result.detection_id`.
  - `marker_ids` is optional — if empty, every detected marker id is kept; if non-empty, only markers whose id is in the list are published/broadcast (across all requested dictionaries). When `detection_id` is auto-generated, the suffix is:
    - empty `marker_ids` -> no suffix
    - a single id `[N]` -> `_id{N}` (e.g. `_id1`)
    - a contiguous range like `[1,2,3]` -> `_idrange{min}_{max}` (e.g. `_idrange1_3`)
    - a non-contiguous list like `[1,3,7]` -> `_customids_minID{min}_maxID{max}` (e.g. `_customids_minID1_maxID7`)
...
|---|---|---|
| `max_processing_threads` | `10` | Max number of concurrent per-topic subscriptions (not detections — each detection gets its own worker thread regardless of this limit). |
...
| `detection_id` | `""` | Optional id; auto-generated from `dictionaries` (and `marker_ids`) if empty. |
| `marker_ids` | `[]` | Marker ids to keep after detection; empty => accept all ids. |
...
| `aruco_start_detection.launch` | `aruco_start_detection` | `action_server_name`, `image_topic`, `dictionaries`, `marker_sizes`, `camera_params_file`, `detection_id`, `marker_ids` |
...
### 4. Dispatch to the right detector in `process_job`

Each job's dedicated worker thread calls `process_job` once per frame.
Replace the single call to `detectMultiDictMarkers` inside it with a
